...
from config import COLOR_MAP_DETECTION
# Yerel modüller
from config import (
...
class Depth3DView:
    ...
    def __init__(self, config: dict):
        ...
        self.preprocessing_socket.connect(
            f"tcp://localhost:{config.get('input_port_preprocessing', PORT_PREPROCESSING_OUTPUT)}")
        ...
        self.calibration_socket.connect(
            f"tcp://localhost:{config.get('input_port_calibration', PORT_CALIBRATION_RESULT)}")
                ...
                if "targets" in message and "timestamp" in message:
                    with self.data_lock:
                        ...
                        for index, props in raw_targets.items():
                            # Pozisyonu index'ten tahmin et? Veya preprocessing eklemeli?
                            ...
                            targets_list.append({
                                ...
                                "size": props.get("amplitude", 0.1) * 0.5  # Boyutu genlikle oranla
                            })
