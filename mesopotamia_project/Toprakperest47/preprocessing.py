...
import argparse
from config import DEFAULT_SDR_SAMPLE_RATE
# Yerel modüller
from config import (
...
# Logging yapılandırması (direkt kullanım için optimize edilmiş)
logger = logging.getLogger("PREPROCESSING")
logger.setLevel(logging.INFO)

# Sadece konsol çıktısı için handler ekle
...
class SignalPreprocessor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # SDR Veri Girişi
        sdr_port = config.get("input_port_sdr", PORT_SDR_DATA)
        self.sdr_data_socket = self.context.socket(zmq.SUB)
        self.sdr_data_socket.connect(f"tcp://localhost:{sdr_port}")
        ...
        cal_port = config.get("input_port_cal", PORT_CALIBRATION_RESULT)
        self.calibration_socket = self.context.socket(zmq.SUB)
        self.calibration_socket.connect(f"tcp://localhost:{cal_port}")
        ...
        out_port = config.get("output_port", PORT_PREPROCESSING_OUTPUT)
        self.output_socket = self.context.socket(zmq.PUB)
        self.output_socket.bind(f"tcp://*:{out_port}")

        # Kontrol Girişi
        ctrl_port = config.get("control_port", PORT_PREPROCESSING_CONTROL)
        self.control_socket = self.context.socket(zmq.REP)
        self.control_socket.bind(f"tcp://*:{ctrl_port}")
