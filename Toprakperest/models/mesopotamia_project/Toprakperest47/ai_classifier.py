...
# Makine öğrenimi kütüphaneleri (opsiyonel, gerektiğinde import edilecek)
...
class AIDetectionResult:
    ...
    id: str # Benzersiz kimlik (örn. timestamp_index)
    type: str # Tespit edilen hedef tipi (örn. METAL, VOID, STONE, UNKNOWN)
    position: Tuple[float, float, float] # Tahmini pozisyon (x, y, z=derinlik) - Şimdilik x,y=0
    size_estimate_m: float # Tahmini boyut (metre cinsinden çap/kenar)
    confidence: float # Tespitin güven skoru (0.0 - 1.0)
    depth_m: float # Hedefin derinliği (metre)
    amplitude: float # Yankının normalize edilmiş veya ham genliği
    phase_change_rad: float # Yankının faz değişimi (radyan)
    shape_estimate: str # Tahmini şekil (point, volumetric, planar)
    timestamp: float = field(default_factory=time.time)
    metadata: Dict[str, Any] = field(default_factory=dict) # Ek bilgiler (frekans vb.)
...
class AIClassifier:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.min_depth_fp_m = float(fp_config.get("min_depth_m", 0.15)) # FP için minimum derinlik
        self.max_size_clutter_m = float(fp_config.get("max_size_clutter_m", 0.1)) # Clutter sayılacak max boyut
        self.min_amplitude_target = float(fp_config.get("min_amplitude_target", 0.05)) # Hedef için min genlik
        self.planar_reflection_phase_threshold_rad = float(fp_config.get("planar_reflection_phase_threshold_rad", 0.2))
        
        # Kural Tabanlı Sınıflandırma Eşikleri (Önceden hesaplanmış)
        rb_config = config.get("rule_based_config", {})
        # Faz eşikleri (radyan cinsinden, +/- pi\ye yakınlık)
        ...
        # Genlik faktörleri (min_amplitude_target\a göre)
        ...
        # Güven skoru hesaplaması için normalizasyon ve taban değerler
        self.metal_conf_amp_norm = self.min_amplitude_target * 3.0 # Metal için daha yüksek genlik beklenir
        self.void_conf_amp_norm = self.min_amplitude_target * 2.0
        self.stone_conf_amp_norm = self.min_amplitude_target * 2.0
        self.base_confidence_boost = 0.05 # Minimum güven artışı
        self.confidence_scale = 1.0 - self.base_confidence_boost # Kalan aralık

        # ZMQ İletişim
        self.context = zmq.Context()
        # Ön İşleme Veri Girişi
        self.input_socket = self.context.socket(zmq.SUB)
        self.input_socket.connect(f"tcp://localhost:{config.get(\"input_port\", PORT_PREPROCESSING_OUTPUT)}")
        self.input_socket.setsockopt_string(zmq.SUBSCRIBE, "")
        self.input_socket.setsockopt(zmq.CONFLATE, 1) # Sadece en son mesajı al
        # AI Sonuç Çıkışı
        self.output_socket = self.context.socket(zmq.PUB)
        self.output_socket.bind(f"tcp://*:{config.get(\"output_port\", PORT_AI_OUTPUT)}")
        # Kontrol Girişi
        self.control_socket = self.context.socket(zmq.REP)
        self.control_socket.bind(f"tcp://*:{config.get(\"control_port\", PORT_AI_CONTROL)}")
        self.control_socket.setsockopt(zmq.RCVTIMEO, 1000)

        # Dahili Durum
        self.running = Event()
        self.shutdown_event = Event()
        self.lock = Lock() # Model veya kritik ayarlara erişim için
        self.model = self._load_model()
        self.threads: Dict[str, Optional[Thread]] = {}

        logger.info(f"AI Sınıflandırıcı başlatıldı (Mod: {'Kural Tabanlı' if self.use_rule_based else 'Model Tabanlı'}).")
