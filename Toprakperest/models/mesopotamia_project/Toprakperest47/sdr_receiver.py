...
try:
    ...
    print("UYARI: SoapySDR kütüphanesi bulunamadı. SDR Alıcı test modunda çalışacak.")
...
dsp_config = config.get("dsp", {})
        ...
        self.notch_freq_hz = float(dsp_config.get("notch_freq_hz", 0))
        self.notch_width_hz = float(dsp_config.get("notch_width_hz", 100e3))
        self.noise_suppression_enabled = bool(config.get("noise_suppression_enabled", True))
        ...
        self.data_socket.bind(f"tcp://*:{config.get(\"output_port\", PORT_SDR_DATA)}")
        self.control_socket = self.context.socket(zmq.REP)
        self.control_socket.bind(f"tcp://*:{config.get(\"control_port\", PORT_SDR_CONTROL)}")
    ...
    def _apply_lms_filter(self, desired_signal: np.ndarray, noise_signal: np.ndarray, initial_weights: np.ndarray, mu: float) -> Tuple[np.ndarray, np.ndarray]:
...
if __name__ == "__main__":
    ...
    if args.config:
        try:
            ...
            logger.error(f"Yapılandırma dosyası bulunamadı: {args.config}. Varsayılanlar ve komut satırı argümanları kullanılıyor.")
        except json.JSONDecodeError as e:
            logger.error(f"Yapılandırma dosyası okunamadı (JSON Hatası): {e}. Varsayılanlar ve komut satırı argümanları kullanılıyor.")
