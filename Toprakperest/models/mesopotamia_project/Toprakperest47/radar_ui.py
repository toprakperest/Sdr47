...
from config import (
    ...
    PORT_AI_CONTROL, PORT_VISUALIZATION_CONTROL, # Bu modülün kontrol portu
    SCAN_FREQUENCY_RANGE_MHZ
)
# 3D Görünüm modülü
...
class RadarMainWindow(QMainWindow):
    ...
    def _handle_preprocessing_message(self, message: dict):
        ...
        with self.data_lock:
            ...
            if self.depth_view:
                 # Depth3DView\ın veriyi işleyecek bir metodu olmalı
                 # Veya UI güncelleme timer\ı bu veriyi kullanır
                 pass 
